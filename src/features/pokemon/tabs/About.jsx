const About = ({pokemon, pokemonSpecies}) => {
    ...
    <>
        <div className='mb-6 border-b pb-4'>
            <h2 className='text-4xl font-extrabold text-gray-800 capitalize tracking-tight'>
                {pokemon.name}
            </h2>
          ...
          {pokemon?.stats?.map((statItem) => {
            ...
            return (
              <div key={statItem.stat.name} className='flex items-center text-sm'>
                {/* Label */}
                <span className='w-24 font-semibold text-gray-500 capitalize truncate'>
                  {statName}
                </span>

                {/* Value */}
                <span className='w-12 font-bold text-gray-800 text-right mr-3'>
                  {val}
                </span>
